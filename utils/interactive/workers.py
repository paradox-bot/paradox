...
def load_into(bot):
    ...
    async def find_user(ctx,
        ...
        if is_member:
            ...
            def is_user(member):
                return ((user_str.lower() in member.display_name.lower())
                        or (user_str.lower() in member.name.lower()))
        else:
        ...
        collection = collection if collection else (
            ctx.server.members if in_server else ctx.bot.get_all_members())
        if maybe_user_id.isdigit():
            user = discord.utils.find(lambda mem: mem.id == maybe_user_id,
                                      collection)
            if user:
                return user
        if interactive:
            ...
            if len(users) > limit:
                await ctx.reply(
                    "Over {} users found matching `{}`! Please refine your search"
                    .format(limit, user_str))
                ctx.cmd_err = (-1, "")
                return None
            if len(users) == 1:
                return users[0]
            if is_member:
                names = [
                    "{} {} {}".format(
                        user.display_name,
                        ("({})".format(user.name)) if user.nick else "",
                        ("({})".format(user.id)) if not in_server else "")
                    for user in users
                ]
            else:
                names = [
                    "{} ({})".format(user.name, user.id) for user in users
                ]
            selected = await ctx.selector(
                "Multiple users found matching `{}`! Please select one.".
                format(user_str), names)
            if selected is None:
                return None
    ...
    async def offer_create_role(ctx, input, timeout=30):
        result = await ctx.ask(
            "Would you like to create this role?", timeout=timeout)
        if result == 0:
            return None
        try:
    ...
    async def find_role(ctx,
        ...
        if userstr == "":
            await ctx.reply(
                "Looking up a role without a name! Something's wacky. Please check your input and try again"
            )
            ctx.cmd_err = (-1, "")
            return None
        ...
        if interactive:
            ...
            def check(role):
                return (role.id == roleid) or (
                    userstr.lower() in role.name.lower())
            ...
            if len(roles) == 0:
                ...
                selected = await ctx.selector(
                    "Multiple roles found matching `{}`! Please select one.".
                    format(userstr), [role.name for role in roles])
                if selected is None:
                    return None
                role = roles[selected]
        else:
        ...
        if role:
            ...
            msg = await ctx.reply(
                "Couldn't find a role matching `{}`!".format(userstr))
            if create:
