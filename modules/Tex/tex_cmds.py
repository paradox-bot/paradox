...
@cmds.cmd("texlisten", category="Maths", short_help="Turns on listening to your LaTeX", aliases=["tl"])
...
def _is_tex(msg):
    return (("$" in msg.clean_content) and 1 - (msg.clean_content.count("$") % 2)
            and msg.clean_content.strip("$")) or ("\\begin{" in msg.clean_content) or ("\\[" in msg.clean_content
                                                                                       and "\\]" in msg.clean_content)


@cmds.cmd("tex", category="Maths", short_help="Renders LaTeX code", aliases=[",", "$", "$$", "align", "latex", "texw"])
...
async def cmd_tex(ctx):
    ...
    if ctx.flags["config"]:
        ...
        if always:
            await ctx.reply("`{0}tex` will now render in math mode. You can use `{0}latex` to render normally.".format(
                ctx.used_prefix))
        else:
    ...
    if ctx.arg_str == "":
        if ctx.used_cmd_name != ",":
            await ctx.reply("Please give me something to compile! See `{0}help` and `{0}help tex` for usage!".format(
                ctx.used_prefix))
        return
...
async def make_latex(ctx):
    source = ctx.msg.clean_content if ctx.objs["latex_listening"] else ctx.msg.clean_content.partition(
    ...
    ctx.objs["latex_name"] = "**{}**:\n".format(ctx.author.name.replace("*", "\\*")) if (await ctx.data.users.get(
    ...
    out_msg = await ctx.reply(
        ...
        message="{}{}".format(ctx.objs["latex_name"],
                              ("Compile Error! Click the {} reaction for details. (You may edit your message)".format(
                                  ctx.objs["latex_show_emoji"])) if error else ""))
    if exists:
        os.remove(file_name)
...
async def show_config(ctx):
    ...
    value_lines = []
    value_lines.append("Keeping your message after compilation"
                       if values[0] or values[0] is None else "Deleting your message after compilation")
    ...
    value_lines.append("Other uses may view your source and errors"
                       if values[3] else "Other users may not view your source and errors")
    value_lines.append("Your name shows on the compiled output"
                       if values[4] or values[4] is None else "Your name is hidden on the compiled output")
    ...
    if len(preamble) > 1000:
        ...
        try:
            await ctx.bot.send_file(
                ctx.author, fp=temp_file, filename="current_preamble.tex", content="Current active preamble")
        except discord.Forbidden:
    ...
    if new_preamble and len(new_preamble) > 1000:
        ...
        try:
            await ctx.bot.send_file(
                ctx.author, fp=temp_file, filename="new_preamble.tex", content="Preamble awaiting approval.")
        except discord.Forbidden:
...
@cmds.cmd("preamble", category="Maths", short_help="Change how your LaTeX compiles", aliases=["texconfig"])
...
async def register_tex_listeners(bot):
    bot.objects["user_tex_listeners"] = [
        str(userid) for userid in await bot.data.users.find("tex_listening", True, read=True)
    ]
    ...
    await bot.log("Loaded {} user tex listeners and {} server tex listeners.".format(
        len(bot.objects["user_tex_listeners"]), len(bot.objects["server_tex_listeners"])))
...
async def tex_listener(ctx):
    ...
    if not (ctx.authid in ctx.bot.objects["user_tex_listeners"] or
            (ctx.server and ctx.server.id in ctx.bot.objects["server_tex_listeners"])):
        return
    if not _is_tex(ctx.msg):
        return
    if ctx.server and (
            ctx.server.id in ctx.bot.objects["server_tex_listeners"]) and ctx.bot.objects["server_tex_listeners"][
                ctx.server.id] and not (ctx.ch.id in ctx.bot.objects["server_tex_listeners"][ctx.server.id]):
        return
    await ctx.bot.log(
