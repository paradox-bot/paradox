...
async def cmd_tag(ctx):
    ...
    if ctx.flags["set"]:
        if len(ctx.flags["set"]) > 5:
            ...
            return
        await ctx.bot.data.users.set(ctx.authid, "custom_prefix",
                                     ctx.flags["set"])
        await ctx.reply(
    ...
    server_prefix = (
        await ctx.server_conf.guild_prefix.get(ctx)) if ctx.server else None
    personal = "Your personal prefix is `{}`.".format(
        personal_prefix
    ) if personal_prefix else "You have not set a personal prefix."
    ...
    await ctx.reply("{}\n{}\n{}".format(personal, server if ctx.server else "",
                                        default))
