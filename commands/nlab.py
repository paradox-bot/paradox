...
async def search_for(string):
    soup = await soup_site(
        search_target.format(urllib.parse.quote_plus(string)))
...
async def cmd_nlab(ctx):
    ...
    """
    direct_page = nlab_url.format("/nlab/show/{}".format(
        urllib.parse.quote_plus(ctx.arg_str)))
    ...
    direct_found = False if not direct_soup.find(
        "title") or "Page not found" in direct_soup.find(
            "title").contents[0] else True
    direct_str = "\nDirect page found at: [{}]({})".format(
        ctx.arg_str, direct_page) if direct_found else ""

    if not "Search results" in soup.find("title").contents[0]:
        await ctx.bot.edit_message(
            out_msg,
            "Nlab directed us to this page which I don't understand:\n{}".
    ...
    if in_title:
        ...
        in_title_str = "\n".join([
            "[{}]({})".format(link[0], nlab_url.format(link[1]))
            for link in in_title
        ][:10])
        emb_fields.append(("{} result{} where query appeared in title".format(
            len(in_title), "" if len(in_title) == 1 else "s"), in_title_str,
                           0))
    if in_body:
        ...
        emb_fields.append(("{} result{} where query appeared in body".format(
            len(in_body), "" if len(in_body) == 1 else "s"), in_body_str, 0))
