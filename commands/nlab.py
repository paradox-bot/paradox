...
async def cmd_nlab(ctx):
    ...
    if in_title:
        ...
        emb_fields.append(("{} results where query appeared in title".format(len(in_title)), in_title_str, 0))
    if in_body:
        in_body = list(in_body)
        in_body_str = "\n".join(["[{}]({})".format(link[0], nlab_url.format(link[1])) for link in in_body][:10])
        emb_fields.append(("{} results where query appeared in body".format(len(in_body)), in_body_str, 0))
