...
async def _config_pages(ctx, serv_conf, value=True):
    ...
    for page in sorted_pages:
        ...
        page_embed = discord.Embed(
            title="{} options:".format(page_name), color=discord.Colour.teal())
        for cat in page_cats:
            if cat not in cats:
                continue
            value_list = []
            for option in cats[cat]:
                value_list.append(await serv_conf[option].hr_get(ctx)
                                  if value else serv_conf[option].desc)
            cat_msg = ctx.prop_tabulate(cats[cat], value_list)

            page_embed.add_field(name=cat, value=cat_msg, inline=False)
        page_embed.set_footer(
            text="Use {}config <option> [value] to see or set an option.".
...
async def cmd_config(ctx):
    ...
    if (ctx.params[0] in ["", "help"]) and len(ctx.params) == 1:
        pages = await _config_pages(
            ctx, serv_conf, value=True if ctx.params[0] == "" else False)
        ...
        if ctx.params[1] not in serv_conf:
            await ctx.reply(
                "Unrecognised option! See `serverconfig help` for all options."
            )
            return
        op = ctx.params[1]
        ...
        if ctx.params[0] not in serv_conf:
            await ctx.reply(
                "Unrecognised option! See `{0.used_prefix}config help` for all options."
                .format(ctx))
            return
        if len(ctx.params) == 1:
            op = ctx.params[0]
            ...
            await serv_conf[ctx.params[0]].hr_set(ctx,
                                                  ' '.join(ctx.params[1:]))
...
async def cmd_rmrole(ctx):
    ...
    result = await ctx.ask(
        "Are you sure you want to delete the role `{}`?".format(role.name))
    if result is None:
    ...
    try:
        ...
        await ctx.reply(
            "I am unable to delete that role, insufficient permissions.")
        return
    except Exception:
...
async def cmd_bancmd(ctx):
    ...
    if not ctx.arg_str:
        bans_str = "Current banned commands: `{}`".format("`, `".join(
            bans)) if bans else "There are no banned commands in this server!"
    ...
    await ctx.reply("{}{}".format(newbanstr if newbans else "",
                                  unbanstr if unbans else ""))
...
@cmds.execute(
    "flags",
    flags=[
        "colour=", "color=", "name==", "perm==", "hoist=", "mention=", "pos=="
    ])
async def cmd_editrole(ctx):
    ...
    if role >= ctx.me.top_role:
        await ctx.reply(
            "The role specified is above or equal to my top role, aborting.")
        return
    if not (ctx.flags["colour"] or ctx.flags["color"] or ctx.flags["name"]
            or ctx.flags["perm"] or ctx.flags["hoist"] or ctx.flags["mention"]
            or ctx.flags["pos"]):
        await ctx.reply(
    ...
    if ctx.flags["colour"] or ctx.flags["color"]:
        colour = ctx.flags["colour"] if ctx.flags["colour"] else ctx.flags[
            "color"]
        hexstr = colour.strip("#")
        if not (len(hexstr) == 6 or all(c in string.hexdigits
                                        for c in hexstr)):
    ...
    if ctx.flags["hoist"]:
        if ctx.flags["hoist"].lower() in ["enable", "yes", "on"]:
            ...
            await ctx.reply(
                "I don't understand your argument to --hoist! See the help for usage."
            )
            return
        edits["hoist"] = hoist
    if ctx.flags["mention"]:
        if ctx.flags["mention"].lower() in ["enable", "yes", "on"]:
            ...
            await ctx.reply(
                "I don't understand your argument to --mention! See the help for usage."
            )
            return
    ...
    if ctx.flags["pos"]:
        ...
        if pos_flag.isdigit():
            ...
            await ctx.reply(
                "I didn't understand your argument to --pos. See the help for usage."
            )
#    msg = ""
...
#            msg += "Moved role to position {}!".format(
            ...
            await ctx.reply(
                "I am unable to move the role, insufficient permissions.")
            return
        except discord.HTTPException:
            await ctx.reply(
                "Something went wrong while moving the role! Possibly I am of too low a rank."
            )
            return
    if edits:
        try:
...
async def channel_cleaner(ctx):
    if not ctx.server:
        return
    if not (ctx.server.id in ctx.bot.objects["cleaned_channels"] and
            ctx.ch.id in ctx.bot.objects["cleaned_channels"][ctx.server.id]):
        return
...
async def register_channel_cleaners(bot):
    ...
    await bot.log("Loaded {} servers with channels to clean.".format(
