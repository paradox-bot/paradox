...
async def cmd_bin2ascii(ctx):
    ...
    asciilist = [
        chr(sum([int(b) << 7 - n for (n, b) in enumerate(byte)]))
        for byte in bytelist
    ]
    await ctx.reply("Output: `{}`".format(''.join(asciilist)))


@cmds.cmd("calc", category="Fun stuff", short_help="Calculator!")
async def cmd_calc(ctx):
    ...
    if response:
        ...
        await ctx.reply(
            "Something went wrong calculating your expression! Please check your input and try again."
        )
        ...
        if r.status == 200:
            ...
            for hit in [random.choice(hits) for i in range(20)]:
                ...
                if "webformatURL" in hit:
                    ...
                    continue
                embed.set_footer(
                    text="Images thanks to the free https://pixabay.com API!")
...
@cmds.cmd("lenny", category="Fun stuff", short_help="( ͡° ͜ʖ ͡°)")
...
async def cmd_dog(ctx):
    ...
    async with aiohttp.get('http://random.dog/woof') as r:
        if r.status == 200:
            dog = await r.text()
            if not (dog.endswith("png") or dog.endswith("jpg")
                    or dog.endswith("gif")):
                await cmd_dog(ctx)
                return
            embed = discord.Embed(
                title="Woof!", color=discord.Colour.light_grey())
...
async def cmd_duck(ctx):
    ...
    async with aiohttp.get(
            'http://random-d.uk/api/v1/quack?type={}'.format(img_type)) as r:
        if r.status == 200:
            js = await r.json()
            embed = discord.Embed(
                title="Quack!", color=discord.Colour.light_grey())
            ...
            await ctx.reply(
                "The ducks are too powerful right now! Please try again later."
            )


@cmds.cmd("sorry", category="Fun Stuff", short_help="Sorry, love.")
async def cmd_sorry(ctx):

    embed = discord.Embed(color=discord.Colour.purple())
    embed.set_image(
        url=
        "https://cdn.discordapp.com/attachments/309625872665542658/406040395462737921/image.png"
    )
    await ctx.reply(embed=embed)
...
async def cmd_cat(ctx, recursion=0):
    ...
    async with aiohttp.get('http://aws.random.cat/meow') as r:
        if r.status == 200:
            js = await r.json()
            embed = discord.Embed(
                title="Meow!", color=discord.Colour.light_grey())
            ...
            if recursion < 10:
                ...
                return
        await ctx.reply(
            "Sorry! The cats are too powerful right now. Please try again later!"
        )


@cmds.cmd("rep", category="Fun stuff", short_help="Give reputation to a user")
async def cmd_rep(ctx):
    ...
    if ctx.arg_str == "" or ctx.arg_str.strip() == "stats":
        ...
        if ctx.arg_str == "":
            ...
            if can_give_in > 0:
                can_give_str = ctx.strfdelta(
                    timedelta(seconds=can_give_in), sec=True)
        ...
        user = await ctx.find_user(
            ctx.arg_str, in_server=True, interactive=True)
        if ctx.cmd_err[0] == -1:
            return
        if not user:
        ...
        if last_rep is not None:
            ...
            if given_ago < cooldown:
                msg = "Cool down! You may give reputation in {}.".format(
                    ctx.strfdelta(
                        timedelta(seconds=(cooldown - given_ago)), sec=True))
                await ctx.reply(msg)
                return
        ...
        await ctx.data.users.set(ctx.authid, "last_rep_time",
                                 str(now.strftime('%s')))
        await ctx.reply("You have given a reputation point to {}".format(
            user.mention))
