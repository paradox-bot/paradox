...
async def log_join(bot, member):
    ...
    embed.set_author(name="New {type} joined: {user} (id: {user.id})".format(type = "bot" if user.bot else "user", user=user),
...
async def log_leave(bot, member):
    ...
    embed = discord.Embed(type="rich", color=colour, description=desc)
    embed.set_author(name="{type} left: {user} (id: {user.id})".format(type = "Bot" if user.bot else "User", user=user),
                     icon_url=user.avatar_url,
                     url=user.avatar_url)
    embed.set_thumbnail(url=user.avatar_url)
    embed.set_footer(text=datetime.utcnow().strftime("Sent at %-I:%M %p, %d/%m/%Y"))
