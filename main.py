...
if os.path.isfile(LOGFILE):
    if os.path.isfile(LOGFILE_LAST):
        shutil.move(
            LOGFILE_LAST, "{}/{}{}.log".format(
                LOGDIR,
                datetime.utcnow().strftime("%s"), LOGNAME))
...
async def log(bot, logMessage):
    ...
    for log in log_splits:
        await bot.send_message(
            discord.utils.get(bot.get_all_channels(), id=LOG_CHANNEL), log)
...
bot.load(
    "commands", "config", "events", "utils", ignore=["RCS", "__pycache__"])

bot.objects["invite_link"] = "http://invite.paradoxical.pw"
bot.objects["support guild"] = "https://discord.gg/ECbUu8u"
bot.objects["sorted cats"] = [
    "General", "Fun Stuff", "Social", "Utility", "User info", "Moderation",
    "Server Admin", "Maths", "Misc"
]

bot.objects["sorted_conf_pages"] = [
...
async def on_ready():
    ...
    await bot.change_presence(
        status=discord.Status.online, game=discord.Game(name=GAME))
    ...
    bot.objects["cheat_report_channel"] = discord.utils.get(
        bot.get_all_channels(), id=CHEAT_CH)
    bot.objects["feedback_channel"] = discord.utils.get(
        bot.get_all_channels(), id=FEEDBACK_CH)
    bot.objects["preamble_channel"] = discord.utils.get(
        bot.get_all_channels(), id=PREAMBLE_CH)
    bot.objects["server_change_log_channel"] = discord.utils.get(
        bot.get_all_channels(), id=BOT_LOG_CH)
